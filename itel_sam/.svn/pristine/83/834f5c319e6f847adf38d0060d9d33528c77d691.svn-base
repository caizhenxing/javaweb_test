function SamDeviceManager(o) {
	...
	var connection = new Ext.data.Connection( {
		url : Ext.ctx + '/samDeviceController/queryForWhereAnd',
		method : 'GET',
		defaultHeaders : {
			'Accept' : 'application/json'
		},
	...
	var store = new Ext.data.JsonStore( {
		...
		remoteSort : true,
		fields : Record,
		sortInfo : { field : 'dateRegTime', direction : 'DESC' },
...
// hiddenName:'deviceType',
	...
	var grid = new Ext.grid.GridPanel( {
		...
		columns : [new Ext.grid.RowNumberer(),
			...
			width : 75,
			sortable : true,
			dataIndex : 'varDeviceId'
		}, {
			header : 'varPassWord',
			width : 75,
			sortable : true,
			dataIndex : 'varPassWord'
		}, {
			header : 'dateRegTime',
			width : 85,
			sortable : true,
			dataIndex : 'dateRegTime'
		}, {
			header : 'varUiTel',
			width : 85,
			sortable : true,
